...
#
# Writes: dist/mccsh-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix mccsh \
  --exe-name Marvel_vs__Capcom__Clash_of_Super_Heroes__Recompiled \
  --display-name "Marvel vs. Capcom: Clash of Super Heroes  Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Marvel vs. Capcom: Clash of Super Heroes disc" \
